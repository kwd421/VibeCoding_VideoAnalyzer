...
class VideoPlayer:
    def __init__(self, canvas_id):
        ...
        try:
            ...
            print(f"[DEBUG __init__] VLC init success, instance={self.instance}, player={self.player}")
    ...
    def set_subtitle(self, sub_path):
        ...
        print(f"[DEBUG set_subtitle] called with: {sub_path}")
        print(f"[DEBUG set_subtitle] vlc_available={self.vlc_available}, player={self.player is not None}, file_exists={os.path.exists(sub_path) if sub_path else False}")
        if self.vlc_available and self.player and sub_path and os.path.exists(sub_path):
            state = self.player.get_state()
            is_playing = self.player.is_playing()
            print(f"[DEBUG set_subtitle] player state={state}, is_playing={is_playing}")
            result = self.player.video_set_subtitle_file(sub_path)
            print(f"[DEBUG set_subtitle] video_set_subtitle_file result={result}")
            spu_count = self.player.video_get_spu_count()
            spu_current = self.player.video_get_spu()
            print(f"[DEBUG set_subtitle] after inject: spu_count={spu_count}, current_spu={spu_current}")
            return True
        print(f"[DEBUG set_subtitle] FAILED - conditions not met")
        return False
