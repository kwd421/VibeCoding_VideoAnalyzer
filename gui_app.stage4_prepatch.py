...
class CustomModelApp:
    ...
    def setup_ui(self):
        ...
        overlay_props.pack_propagate(False)
        tk.Label(overlay_props, text='Overlay Properties', bg=C['bg2'], fg=C['text'], font=('Noto Sans KR', 10, 'bold')).pack(anchor='w', padx=12, pady=(12, 8))
    ...
    def _load_overlay_photo(self, item, width, height, draft=False):
        cache_key = (item.id, item.type, int(width), int(height), item.text or '', round(item.opacity, 3), int(item.font_size), item.text_color, bool(draft))
        cached = self._overlay_preview_cache.get(cache_key)
        if cached is not None:
            return cached
        if item.type == 'image':
            img = self._get_overlay_source_image(item.source)
            if img is None:
                return None
