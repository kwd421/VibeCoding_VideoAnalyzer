...
import numpy as np
import vlc
from app.engine.engine_core import HyperTranscriptionEngine, HAS_WHISPERX
from app.media.video_editor import VideoEditor
from app.ui.video_player import VideoPlayer
from app.core.config_models import AnalysisSettings
from app.core.timeline_manager import TranscriptManager
from app.core.event_dispatcher import EventEmitter
from app.engine.analysis_controller import AnalysisController
from app.ui.ui_block_editor import UIBlockEditor
from app.core.transcription_backends import BACKEND_LABELS, mode_from_label
from app.core.subtitle_cues import (
    SubtitleRenderStyle,
    build_subtitle_cues,
    cues_to_entries,
    cues_to_srt,
...
class CustomModelApp:
    ...
    def setup_ui(self):
        ...
        bf = tk.Button(c1, text='  📂  영상 파일 선택  ', command=self.on_select_video, bg=C['surface'], fg=C['text'], font=_f, relief='flat', bd=0, compound='center', pady=8, cursor='hand2'); bf.pack(fill=tk.X, pady=(0,6))
        ...
        self.device_var = tk.StringVar(value='Apple Core ML (실험)'); ttk.Combobox(r, textvariable=self.device_var, values=BACKEND_LABELS, state='readonly', width=18).pack(side=tk.RIGHT)
        ...
        self.btn_analyze = tk.Button(c2, text='  분석 시작  ', command=self.on_start_analysis, bg=C['surface'], fg=C['text'], font=('Noto Sans KR', 13, 'bold'), relief='flat', bd=0, compound='center', pady=10, cursor='hand2'); self.btn_analyze.pack(fill=tk.X, pady=(0,4))
        self.btn_stop = tk.Button(c2, text='  작업 중지  ', command=self.on_stop_action, bg=C['surface'], fg=C['text'], disabledforeground=C['text'], font=_fb, relief='flat', bd=0, compound='center', state=tk.DISABLED, pady=5, cursor='hand2'); self.btn_stop.pack(fill=tk.X)
        ...
        bx = tk.Button(c4, text='  📄 자막 내보내기  ', command=self.export_subtitles, bg=C['surface'], fg=C['text'], font=_fb, relief='flat', bd=0, compound='center', pady=8, cursor='hand2'); bx.pack(fill=tk.X, pady=(8,0))
    ...
    def reset_action_button(self): self.save_frame.pack_forget(); self.btn_analyze.config(text="  분석 시작  ", command=self.on_start_analysis, bg=self.C['surface'], fg=self.C['text'], disabledforeground=self.C['text'], state=tk.NORMAL); self.btn_fast_save.config(state=tk.NORMAL); self.btn_pro_save.config(state=tk.NORMAL)
